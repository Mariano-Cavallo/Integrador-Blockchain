...
__global__ void bruteforce_kernel(
    ...
    int            prefix_len,
    uint64_t       range_start,  // nonce minimo de esta tanda
    uint64_t       range_end,    // nonce maximo global (inclusive)
    int*           found_flag,
    uint64_t*      found_nonce,
    uint32_t*      found_digest
) {
    uint64_t nonce = range_start
                   + (uint64_t)blockIdx.x * blockDim.x
                   + threadIdx.x;

    // Respetar el limite superior del rango
    if (nonce > range_end) return;
...
bool parse_prefix(const char* hex, uint8_t* nibbles, int* len) {
    *len = (int)strlen(hex);
    if (*len > 32) { fprintf(stderr, "Prefijo demasiado largo (max 32)\n"); return false; }
    for (int i = 0; i < *len; i++) {
        char c = hex[i];
...
int main(int argc, char* argv[]) {
    if (argc != 5) {
        printf("Uso: %s <cadena> <prefijo_hex> <nonce_min> <nonce_max>\n", argv[0]);
        printf("Ejemplo: %s hola 0000 0 100000\n", argv[0]);
        return 1;
    }

    const char* cadena    = argv[1];
    const char* prefijo   = argv[2];
    uint64_t    nonce_min = (uint64_t)strtoull(argv[3], NULL, 10);
    uint64_t    nonce_max = (uint64_t)strtoull(argv[4], NULL, 10);

    if (nonce_min > nonce_max) {
        fprintf(stderr, "Error: nonce_min debe ser <= nonce_max\n");
        return 1;
    }
    ...
    printf("Buscando nonce en [%llu, %llu] para MD5(%s + nonce) que empiece con '%s'...\n",
           (unsigned long long)nonce_min, (unsigned long long)nonce_max, cadena, prefijo);
    fflush(stdout);

    // --- diagnostico: GPU detectada y su compute capability ---
    int devCount = 0;
    cudaError_t cntErr = cudaGetDeviceCount(&devCount);
    if (cntErr != cudaSuccess || devCount == 0) {
        fprintf(stderr, "ERROR: no se detecta GPU CUDA: %s (devices=%d)\n",
                cudaGetErrorString(cntErr), devCount);
        return 4;
    }
    cudaDeviceProp prop;
    ...
    uint64_t start = nonce_min;

    while (h_flag == 0 && start <= nonce_max) {
        ...
        // --- chequeo de errores CUDA (diagnostico) ---
        cudaError_t launchErr = cudaGetLastError();
        if (launchErr != cudaSuccess) {
            fprintf(stderr, "ERROR lanzando kernel: %s\n", cudaGetErrorString(launchErr));
            return 2;
        }
        cudaError_t syncErr = cudaDeviceSynchronize();
        if (syncErr != cudaSuccess) {
            fprintf(stderr, "ERROR en cudaDeviceSynchronize: %s\n", cudaGetErrorString(syncErr));
            return 3;
        }
